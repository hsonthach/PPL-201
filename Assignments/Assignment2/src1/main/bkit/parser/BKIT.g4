...
// Ex : a,b=5,c,d[3]
variable_list: variable_assign (COMMA variable_assign)*;

// Ex: a = 5  | a | a[5] = {1,2,3} 
variable_assign: variable (ASSIGN literals)?;
...
body: BODY COLON statement* END_BODY DOT;
 ...
 */
if_stmt:
	IF exp THEN statement* (else_if_stmt)* (else_stmt)? END_IF DOT;

/*
 Ex:ElseIf expression Then statement-list
 */
else_if_stmt: ELSE_IF exp THEN statement*;
 ...
 */
for_stmt:
	FOR LP ID ASSIGN exp COMMA exp COMMA exp RP DO statement* END_FOR DOT;

//********* 3.4 While Statement
/*
 Ex:While expression Do statement-list EndWhile.
 */
while_stmt: WHILE exp DO statement* END_WHILE DOT;
 ...
 */
do_while_stmt: DO statement* WHILE exp END_DO DOT;
...
// { "abc", 3 }
array_literal: LCP (literals (COMMA literals)*) RCP;
...
// Index ID (LSB exp RSB)+;
index_exp: index_exp index_operators | exp7;
// TODO: search elearning & doc & stackoverflow  about index 
index_operators: LSB exp RSB;
// Function call
exp7: ID LP exps_list RP | exp8;
// Parentheses
exp8: LP exp RP | ID | literals;
...
LSB: '['; // Square bracket left 

RSB: ']'; // Square bracket right 
...
ID:
	[a-z][_a-zA-Z0-9]*; // ID have to be after key words to avoiding misunderstanding for antlr
